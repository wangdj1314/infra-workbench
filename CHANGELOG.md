...
- **后端**：新增 `ITOP_STIMULUS_TOOL` 映射——已知流转动作优先路由到对应专用工具（报错更准、字段校验内置），缺团队/工程师/解决方案/挂起原因时提前 400 提示；专用工具失败自动降级通用 `apply_stimulus` + 字段自适应重试（兼容旧版 MCP）；新增 `GET /api/itop/options`：团队/处理工程师/服务族/服务/服务子类别下拉数据（OQL friendlyname，10 分钟缓存，逐类容错）；`/transitions` 接口同步下发 `resolution_codes`/`pending_reasons` 枚举
