...
- **背景**：itop-mcp 升级后新增了一批专用流转工具（`assign_ticket`/`reassign_ticket`/`process_ticket`/`resolve_ticket`/`pending_ticket`/`close_ticket`，内置字段校验），且明确了「解决方式/挂起原因」枚举取值；工作台同步适配
- **后端**：新增 `ITOP_STIMULUS_TOOL` 映射——已知流转动作优先路由到对应专用工具（报错更准、字段校验内置），缺团队/工程师/解决方案/挂起原因时提前 400 提示；专用工具失败自动降级通用 `apply_stimulus` + 字段自适应重试（兼容旧版 MCP）；新增 `GET /api/itop/options`：团队/处理工程师/服务族/服务/服务子类别下拉数据（OQL friendlyname，分页拉全——run_oql 单页上限 200 条而人员超千条，10 分钟缓存，逐类容错）；`/transitions` 接口同步下发 `resolution_codes`/`pending_reasons` 枚举
- **前端**：流转弹窗中原需手输 ID 的「处理团队/处理工程师/服务族/服务/服务子类别」全部改为下拉选项框（异步加载，失败降级手输）；「解决方式」「挂起原因」按 MCP 内置枚举修正为下拉选项（日常运维/协助、已修复、临时方案、已知错误、其他；等待用户反馈/变更/供应商、其他）
- 版本徽标 → v29.8

### 同日补丁：必填字段默认值自动补全（不再拦截流转）
- 流转时 iTop 报「Missing mandatory attribute(s)」（如解决工单要求 servicefamily_id/service_id）时，不再弹窗拦截，而是自动补默认值后重试：服务族/服务/子类别优先取工单原值（raw_data），否则取默认配套值（优先「事件处理/日常运维」族，族→服务→子类别逐层配套选取，可用 `ITOP_DEFAULT_SERVICEFAMILY_ID/ITOP_DEFAULT_SERVICE_ID/ITOP_DEFAULT_SERVICESUBCATEGORY_ID` 环境变量覆盖）；解决方式默认「日常运维/协助」、挂起原因默认「其他」；其余字段尝试工单原值
- 提交备注自动追加「必填字段由工作台默认值自动补全：xxx」便于审计；同一缺失组合只自动补一次，补后仍被拒则回退前端动态补填弹窗，不会死循环
- 选项接口服务/子类别带上父级外键（servicefamily_id/service_id）以支撑配套选取
...
### 修复：任务完成说明不回显（同日补丁）
- 完成任务时填写的「完成情况简要说明」已存入 `completion_note`，但工作项卡片从未渲染该字段，完成后看不到说明；现主任务/子任务卡片均回显绿色「完成说明」块，且提交完成时立即回显（不等整页刷新）

### 交互：弹窗不再支持点击遮罩关闭（同日补丁）
- 生成 Token 等弹窗内复制文本时，按 Ctrl/点选易误触遮罩导致弹窗消失；现 `showModal` 遮罩不再响应点击关闭，统一只能经右上角 × 或取消按钮关闭

### 新增：里程碑编辑功能（同日补丁）
- 里程碑记录支持编辑：列表条目新增 ✎ 按钮（登记人本人或管理员可见），弹窗回填已登记的状态与说明，标题切为「编辑里程碑状态」，提交走 PUT 接口；编辑同样记录操作日志并联动刷新工作项更新时间

### 新增：侧边栏「沟通建议」（同日补丁）
- AI 工作建议区新增「💬 沟通建议」按钮：拉取近 7 天钉钉聊天记录（单条截断、总量控制防超 token），AI 分析后给出待回复/待确认事项识别 + 可直接使用的话术建议（开场/要点/收尾）+ 沟通习惯改进提示；结果展示在工作建议框内，用量统计登记为 `comm_suggest`

### 新增：里程碑弹窗 AI 辅助增强（同日补丁）
- 「登记里程碑状态」弹窗：「当前状态」旁新增 **AI 总结**（根据已填内容或补充说明生成 20 字以内状态短语直接填入）；补充说明区拆为 **AI 补充**（未填写时生成，已有内容时提示改用润色）与 **AI 润色**（已有内容时润色，结果展示在可编辑预览框，确认后点「📥 填入说明」才写入，同完成说明润色流程）；用量统计登记 `ms_summary`/`ms_polish`
...
### 新增：工作项筛选增强 + AI 语义搜索
- 个人工作台与「工作内容管理」筛选栏在原有状态筛选基础上，新增 **分类筛选** 与 **优先级筛选**（后端 `/api/work-items` 支持 `category`/`priority` 参数）
- 筛选栏右侧新增 **🔍 AI 搜索框**：输入自然语言描述（如“上周那个服务器的事”“谁在跟进堡垒机”），后端先 SQL 关键词粗筛候选（标题/描述/分类，限 120 条）再交 AI 语义判断相关性，返回匹配事项与理由；搜索结果以列表过滤 + 顶部提示条展示，可一键清除；用量统计登记为 `search`

### 修复：AI 聊天分析区分消息收发方向
- 沟通建议、工作建议、岗位分析等所有涉及钉钉聊天记录分析的 AI 模块：素材组装时标注每条消息的收发方向（「我发出」/「对方发出」，按发送人与本人 display_name 比对），提示词明确要求 AI 区分哪些话是用户说的、哪些是接收到的，重点识别“对方发出但用户尚未回应”的事项，避免把对方的表态误当成用户的承诺，显著提升分析准确性
